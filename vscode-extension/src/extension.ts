...
class AIAssistantProvider {
    ...
    async sendMessage(message: string, context?: any): Promise<string> {
        ...
        try {
            // Build context
            const contextData = await this.buildContext(context);
...
${contextData}`
    ...
    private async buildContext(context?: any): string {
        if (!this.config.contextAwareness) return '';
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.type) {
                case 'sendMessage':
                    try {
                        const response = await this.aiAssistant.sendMessage(message.text);
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('aiAssistant.chatView', chatProvider)
    );
    ...
    const explainCodeCommand = vscode.commands.registerCommand('aiAssistant.explainCode', async () => {
    ...
    const debugCodeCommand = vscode.commands.registerCommand('aiAssistant.debugCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor found');
            return;
        ...
        try {
            const response = await aiAssistant.sendMessage(
                `Please debug and analyze this ${language} code for potential issues, bugs, and improvements:\n\n\`\`\`${language}\n${selectedText}\n\`\`\``
            );
    ...
    const refactorCodeCommand = vscode.commands.registerCommand('aiAssistant.refactorCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor found');
            return;
        ...
        try {
            const response = await aiAssistant.sendMessage(
                `Please refactor and optimize this ${language} code for better performance, readability, and best practices:\n\n\`\`\`${language}\n${selectedText}\n\`\`\``
            );
    ...
    const generateCodeCommand = vscode.commands.registerCommand('aiAssistant.generateCode', async () => {
        ...
        try {
            const response = await aiAssistant.sendMessage(
                `Please generate code based on this description: ${prompt}\n\nInclude explanations and best practices.`
            );
    ...
    const switchModelCommand = vscode.commands.registerCommand('aiAssistant.switchModel', async () => {
        const modelOptions = FREE_MODELS.map(model => ({
            label: model.split('/')[1]?.replace(':free', '') || model,
            description: model,
            detail: 'Free model'
        }));
    ...
    vscode.window.showInformationMessage(
        'AI Coding Assistant Pro activated! 🚀',
        'Open Chat',
        'View Commands'
    ).then(selection => {
